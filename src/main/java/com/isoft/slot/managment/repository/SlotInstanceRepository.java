...
public interface SlotInstanceRepository extends JpaRepository<SlotInstance, Long> {
    ...
    @Query("SELECT slot from SlotInstance slot where slot.temp.id=:tempId and slot.timeFrom>=:timeFrom and slot.timeTo<=:timeTo and slot.centerId=:centerId and slot.availableCapacity>0")
